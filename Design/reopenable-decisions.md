...
What is NOT here: items still genuinely open (the Observatory + Protostar icon
ladders, the missing `dmarc@` mailbox), and decisions closed for good with no
...
## 9. AI crawlers are allowed everything, and named explicitly

**Decision.** `public/robots.txt` keeps `User-agent: * / Allow: /` and adds a
second group naming sixteen AI crawler tokens on the same `Allow: /`. A
generated `/llms.txt` states the company once for whatever fetches it.

**Why.** Being cited by an answer engine is discovery this studio cannot buy,
and the retrieval indexes behind those engines are the same search indexes the
site now submits to. The named group changes no behaviour today — `*` already
permits everything, and an agent absent from the list falls through to it — so
what it buys is that the stance is a decision on the record rather than a
default nobody chose.

The sixteen fall in three kinds and the trade is different for each: search
crawlers building a retrieval index (`OAI-SearchBot`, `Claude-SearchBot`,
`PerplexityBot`), user-triggered fetchers pulling a page because someone asked
about the company in a chat (`ChatGPT-User`, `Claude-User`, `Perplexity-User`,
`MistralAI-User`), and training crawlers whose corpora are how a model knows the
company exists at all (`GPTBot`, `ClaudeBot`, `CCBot`). `Google-Extended` and
`Applebot-Extended` are not crawlers — they are opt-out switches over content
Googlebot and Applebot already hold, governing whether it may ground AI
Overviews / Gemini and Apple Intelligence.

**The footgun, which the file itself no longer states.** The comments explaining
all of the above were stripped on the owner's instruction: a directive file read
by machines is the wrong place for the reasoning. One consequence has to live
somewhere, and this is it — **robots.txt groups do not merge.** A crawler
matching the named group reads that group and ignores `*` entirely. Any
`Disallow` added to `*` later will NOT apply to these sixteen unless it is
repeated in their group, and they are the crawlers most likely to be quoting the
site when it silently fails to.

**Trigger to reopen.** Evidence the trade has inverted — content reproduced at
length without attribution, or measurable referral loss to an answer engine that
summarises instead of linking. The lever is per-kind, not all-or-nothing:
`Disallow` the training crawlers while keeping the search and citation ones, or
flip only `Google-Extended` / `Applebot-Extended`, which removes AI-answer
grounding without touching normal search ranking. Reversing all sixteen at once
would be answering a question nobody asked.

**Confidentiality rail on `/llms.txt`, which applies whether or not this
reopens.** The file is generated by `src/pages/llms.txt.ts` from the same
`src/data` the pages render, and takes **only `blurb`, never `long` or
`detail`** — decision 8's rail, applied to a plain-text file that is fetched
precisely because it is easy to fetch. Product status travels with every entry
and the Credits attribution is restated in full, because the failure this file
exists to prevent is not obscurity but confident misdescription: an
ideation-stage game announced as released, or a team member's credit at another
studio attributed to the company.
